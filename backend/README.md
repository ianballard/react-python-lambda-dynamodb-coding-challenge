...
```bash
backend$ sam local invoke TodoFunction --event events/event.json
```
...
```bash
backend$ sam logs -n TodoFunction --stack-name "backend" --tail
```
...
backend$ AWS_SAM_STACK_NAME="backend" python -m pytest tests/integration -v
```
